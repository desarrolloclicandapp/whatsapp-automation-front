    ...
    const fetchSubscriptions = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const nextSubscriptions = Array.isArray(data) ? data : [];
                setSubscriptions(nextSubscriptions);
                if (nextSubscriptions.length === 0) setShowPlans(true);
            }
        } catch {
        ...
        <>
            <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex flex-col gap-4">
                    <nav className="flex gap-6 overflow-x-auto border-b border-gray-200 dark:border-gray-800" aria-label="Secciones de suscripción">
                        {['services', 'payments', 'invoices'].map(tab => (
                            <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`shrink-0 border-b-2 px-1 pb-3 pt-1 text-sm font-semibold transition-colors ${activeTab === tab ? 'border-indigo-600 text-indigo-600 dark:border-indigo-400 dark:text-indigo-300' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-800 dark:text-gray-400 dark:hover:border-gray-700 dark:hover:text-gray-200'}`}>{tab === 'services' ? t('sub.tab.services') : tab === 'payments' ? t('sub.tab.payments') : t('sub.tab.invoices')}</button>
                        ))}
                    </nav>
                </div>
                                            ...
                                            <div key={sub.id} className="transition hover:bg-gray-50 dark:hover:bg-gray-800/30">
                                                ...
                                                {isEditing && (
                                                    <div className="px-6 pb-6 animate-in slide-in-from-top-2 fade-in">
                                                        <div className="border-t border-indigo-200 pt-5 dark:border-indigo-900">
                            ...
                            subscriptions.length > 0 && hasAddonAccess && (
                                <div className="space-y-4 pt-6 border-t border-gray-200 dark:border-gray-800">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {/* SUBAGENCIA EXTRA */}
                                        <div className="border-t border-gray-200 py-5 flex items-center justify-between gap-4 transition group dark:border-gray-800">
                                            <div className="flex items-center gap-4"><div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-xl"><Building2 size={24} /></div><div><h4 className="font-bold text-gray-900 dark:text-white">{isChatwootAgency ? (t('sub.extras.cw_account') || "Cuenta WaFloW Inbox extra") : t('sub.extras.subagency')}</h4><p className="text-xs text-gray-500">{isChatwootAgency ? (t('sub.extras.cw_account_desc') || "Añade una cuenta independiente de WaFloW Inbox") : t('sub.extras.subagency_desc')} · {hasVolumeDiscount ? t('sub.extras.vip_pack') : t('sub.extras.regular')}</p></div></div>
